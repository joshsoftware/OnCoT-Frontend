...
import { driveTimerAction, driveTimerRequestFailed } from 'actions/driveTimerActions';

import { DRIVE_TIMER } from 'constants/actionConstants';

export function* driveTimer() {
  try {
    const { data } = yield call(getDriveTimer);
    yield put(driveTimerAction(data));
  } catch (error) {
    yield put(driveTimerRequestFailed('Something went wrong with drive timer!'));
  }
}
